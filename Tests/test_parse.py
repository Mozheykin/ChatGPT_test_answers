...
class TestParse(unittest.TestCase):
    def test_question(self):
        self.assertEqual(get_question_text(OUT1[0]), 'Ss')
    
    def test_question1(self):
        self.assertEqual(get_question_text(OUT2[0]), 'The interaction of one drug increased by the presence of a second drug is known as:')

    def test_question2(self):
        self.assertEqual(get_question_text(OUT4[0]), 'What is the reason behind placing the bed in the reverse Trendelenburg position while the nurse cares for a patient with critical limb ischemia?')
